...
<div class="facilities-list" style="display: flex; flex-wrap: wrap; gap: 28px;">
	@forelse($facilities as $facility)
		<div class="facility-card" style="background:#fff; border-radius:16px; box-shadow:0 4px 18px rgba(0,0,0,0.08); padding:22px 18px; width:320px; display:flex; flex-direction:column; align-items:center;">
			@if($facility->image)
				<img src="{{ asset('storage/' . $facility->image) }}" alt="Facility" style="width:100%; height:140px; object-fit:cover; border-radius:10px; margin-bottom:18px;">
			@else
			...
			<div style="font-size:0.98rem; color:#555; margin-bottom:10px;">{{ $facility->address ?? '-' }}</div>
				<div style="display:flex; gap:10px; align-items:center;">
					<a href="{{ route('facilities.show', $facility->id) }}" class="action-btn-view" style="position:relative; color:#6366f1; font-size:1.2rem; display:inline-flex; align-items:center; text-decoration:none;">
						<i class="fa fa-eye"></i>
					...
					@if($userRole !== 'staff')
						<a href="{{ route('facilities.edit', $facility->id) }}" class="action-btn-edit" style="position:relative; color:#6366f1; font-size:1.2rem; display:inline-flex; align-items:center; text-decoration:none;">
							<i class="fa fa-pen"></i>
						...
						<form action="{{ route('facilities.destroy', $facility->id) }}" method="POST" style="display:inline; margin:0; position:relative;">
							@csrf
							@method('DELETE')
					...
					<a href="{{ url('/modules/facilities/' . $facility->id . '/energy-profile') }}" class="energy-profile-btn" style="position:relative; color:#f59e42; font-size:1.25rem; display:inline-flex; align-items:center; text-decoration:none;">
