...
@if(isset($facilityModel) && $facilityModel)
    ...
    <table style="width:100%;border-collapse:collapse;min-width:400px;">
        ...
        <tbody>
            ...
            <tr style="border-bottom:1px solid #e5e7eb;">
                <td style="padding:10px 14px;font-weight:600;color:#2563eb;">{{ $label }}</td>
                <td style="padding:10px 14px;">{{ $first3mo?->{'month'.$num} ?? '-' }}</td>
                <td style="padding:10px 14px;">
                    @if($first3mo && $first3mo->{'month'.$num} !== null)
                    <form method="POST" action="{{ route('facilities.first3months.delete', ['facility_id' => $facilityModel->id ?? request('facility_id'), 'month_no' => $num]) }}" style="display:inline;" class="delete-month-form">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn-delete-month" data-month="{{ $num }}" style="background:#ef4444;color:#fff;border:none;padding:6px 16px;border-radius:7px;font-weight:600;cursor:pointer;">Delete</button>
                    </form>
                    @endif
                </td>
            </tr>
    ...
    <div style="display:flex;justify-content:center;align-items:center;width:100vw;height:100vh;">
        <div style="background:#fff;padding:32px 28px;border-radius:16px;min-width:320px;box-shadow:0 8px 32px rgba(31,38,135,0.13);position:relative;max-width:90vw;">
            ...
            <form method="POST" action="{{ route('facilities.first3months.store') }}" autocomplete="off">
                ...
                <div class="form-group" style="margin-bottom:18px;">
                    ...
                    <select name="month_no" id="month_no" class="form-control" required style="width:100%;padding:10px 12px;border-radius:7px;border:1.5px solid #c7d2fe;font-size:1.08rem;">
                        ...
                        @foreach($monthLabels as $num => $label)
                            ...
                            @else
                                <option value="{{ $num }}">{{ $label }} (current: {{ $currentVal }})</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                ...
                <input type="hidden" name="facility_id" value="{{ $facilityModel->id ?? request('facility_id') ?? '' }}">
...
@endphp
@include('modules.facilities.energy-profile.partials.modals', ['avgKwh' => $avgKwh])
