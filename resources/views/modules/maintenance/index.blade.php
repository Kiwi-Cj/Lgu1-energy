...
<div style="max-width:1100px;margin:0 auto;">
    ...
    <div class="row" style="display:flex;gap:24px;flex-wrap:wrap;margin-bottom:2rem;">
        <div class="card" style="flex:1 1 220px;min-width:220px;background:#fff0f3;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(225,29,72,0.08);">
            ...
            <div style="font-size:2rem;font-weight:700;margin:8px 0;">{{ $needingCount ?? 0 }}</div>
        </div>
        <div class="card" style="flex:1 1 220px;min-width:220px;background:#fef9c3;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(234,179,8,0.08);">
            ...
            <div style="font-size:2rem;font-weight:700;margin:8px 0;">{{ $pendingCount ?? 0 }}</div>
        </div>
        <div class="card" style="flex:1 1 220px;min-width:220px;background:#f0fdf4;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(34,197,94,0.08);">
            ...
            <div style="font-size:2rem;font-weight:700;margin:8px 0;">{{ $ongoingCount ?? 0 }}</div>
        </div>
        <div class="card" style="flex:1 1 220px;min-width:220px;background:#e0f7fa;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(0,188,212,0.08);">
            ...
            <div style="font-size:2rem;font-weight:700;margin:8px 0;">{{ $completedCount ?? 0 }}</div>
        </div>
    </div>
    <!-- FILTERS -->
    ...
    <table class="table" style="width:100%;margin-top:12px;background:#fff;border-radius:14px;overflow:hidden;text-align:center;box-shadow:0 2px 12px rgba(55,98,200,0.07);border-collapse:separate;border-spacing:0;">
        ...
        <tbody>
            ...
            <tr data-id="{{ $row['id'] ?? $i }}" data-maintenance_type="{{ $row['maintenance_type'] ?? '' }}" data-scheduled_date="{{ $row['scheduled_date'] ?? '' }}" data-assigned_to="{{ $row['assigned_to'] ?? '' }}" data-completed_date="{{ $row['completed_date'] ?? '' }}" style="background:@if($loop->index%2==0)#f9fafb;@else#fff;@endif;transition:background 0.2s;" onmouseover="this.style.background='#e0e7ff'" onmouseout="this.style.background='{{ $loop->index%2==0 ? '#f9fafb' : '#fff' }}'">
                <td style="padding:12px 8px;font-weight:500;">{{ $row['facility'] }}</td>
                <td style="padding:12px 8px;">{{ $row['issue_type'] }}</td>
                <td style="padding:12px 8px;">{{ $row['trigger_month'] }}</td>
                <td style="padding:12px 8px;">{{ $row['efficiency_rating'] }}</td>
                <td style="padding:12px 8px;">{{ $row['maintenance_status'] }}</td>
                <td style="padding:12px 8px;">{{ $row['scheduled_date'] }}</td>
                <td style="padding:12px 8px;">{{ $row['remarks'] ?? '-' }}</td>
                <td style="padding:12px 8px;">{!! str_replace('btn btn-sm', 'btn btn-sm schedule-btn', $row['action']) !!}</td>
            </tr>
            @empty
