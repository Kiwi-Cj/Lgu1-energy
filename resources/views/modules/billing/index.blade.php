...
<div class="row" style="display:flex;gap:18px;flex-wrap:wrap;margin-bottom:2rem;">
	<div class="card" style="flex:1 1 180px;min-width:180px;background:#f5f8ff;padding:18px 14px;border-radius:12px;box-shadow:0 2px 8px rgba(55,98,200,0.08);">
		...
		<div style="font-size:1.5rem;font-weight:700;margin:8px 0;">{{ $totalFacilitiesBilled ?? '-' }}</div>
	</div>
	<div class="card" style="flex:1 1 180px;min-width:180px;background:#f5f8ff;padding:18px 14px;border-radius:12px;box-shadow:0 2px 8px rgba(55,98,200,0.08);">
		<div style="font-size:1.05rem;font-weight:500;color:#3762c8;">Total Amount Billed (PHP)</div>
		<div style="font-size:1.5rem;font-weight:700;margin:8px 0;">{{ $totalAmountBilled ?? '-' }}</div>
	</div>
	<div class="card" style="flex:1 1 180px;min-width:180px;background:#f5f8ff;padding:18px 14px;border-radius:12px;box-shadow:0 2px 8px rgba(55,98,200,0.08);">
		<div style="font-size:1.05rem;font-weight:500;color:#3762c8;">Facilities with Pending Bills</div>
		<div style="font-size:1.5rem;font-weight:700;margin:8px 0;">{{ $pendingFacilities ?? '-' }}</div>
	</div>
</div>
<form method="GET" action="" style="margin-bottom:18px;display:flex;gap:14px;align-items:end;flex-wrap:wrap;">
	...
	<div style="display:flex;flex-direction:column;">
		...
		<input type="month" name="month" id="month" class="form-control" value="{{ $filterMonth ?? '' }}" style="min-width:110px;padding:6px 10px;border-radius:7px;border:1px solid #c3cbe5;font-size:1rem;">
	</div>
	<div style="display:flex;flex-direction:column;">
...
<table class="table" style="width:100%;background:#fff;border-radius:10px;overflow:hidden;text-align:center;">
	...
	<tbody>
		...
		<tr>
			<td>{{ $bill->facility->name ?? '-' }}</td>
			<td>{{ $bill->month ? date('M Y', strtotime($bill->month.'-01')) : '-' }}</td>
			<td>{{ $bill->kwh_consumed }}</td>
			<td>{{ $bill->unit_cost }}</td>
			<td>{{ $bill->total_bill }}</td>
			<td>
				<span style="font-weight:600;color:
					@if($bill->status == 'Paid') #22c55e;
					@elseif($bill->status == 'Pending') #eab308;
					@else #e11d48;
					@endif">
					{{ $bill->status }}
				</span>
			...
			<td style="display:flex;gap:10px;align-items:center;justify-content:center;">
				   <div class="action-btn-group" style="display:flex;gap:10px;align-items:center;position:relative;">
					   <div class="action-tooltip-container" style="position:relative;">
						   <a href="{{ route('modules.billing.show', $bill->id) }}" class="action-btn-view action-btn-tooltip" data-tooltip="View" style="color:#6366f1;font-size:1.2rem;display:inline-flex;align-items:center;text-decoration:none;position:relative;">
							   <i class="fa fa-eye"></i>
					   ...
					   <div class="action-tooltip-container" style="position:relative;">
						   <a href="{{ route('modules.billing.edit', $bill->id) }}" class="action-btn-edit action-btn-tooltip" data-tooltip="Edit" style="color:#6366f1;font-size:1.2rem;display:inline-flex;align-items:center;text-decoration:none;position:relative;">
							   <i class="fa fa-pen"></i>
					   ...
					   <div class="action-tooltip-container" style="position:relative;">
						   <form action="{{ route('modules.billing.destroy', $bill->id) }}" method="POST" style="display:inline;margin:0;">
							   @csrf
							   @method('DELETE')
