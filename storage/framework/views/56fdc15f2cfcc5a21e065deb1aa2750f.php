...
<div style="max-width:1200px;margin:0 auto;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
        <h2 style="font-size:2rem;font-weight:700;color:#222;">Energy Profile - <?php echo e($facilityModel->name ?? ''); ?></h2>
        <button type="button" class="btn-add-energy-profile" 
            style="background:linear-gradient(90deg,#2563eb,#6366f1);color:#fff;padding:10px 28px;font-weight:600;border:none;border-radius:10px;box-shadow:0 2px 8px rgba(31,38,135,0.1);font-size:1.05rem;transition:0.2s; <?php if(!$hasFirst3mo || $energyProfiles->count()): ?> opacity:0.5; pointer-events:none; <?php endif; ?>"
            <?php if(!$hasFirst3mo): ?> disabled title="You need first 3 months data before adding an energy profile." 
            <?php elseif($energyProfiles->count()): ?> disabled <?php endif; ?>>+ Add Energy Profile</button>
    </div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:20px;margin-bottom:1.5rem;">
        <div style="flex:1 1 220px;background:#f0fdf4;padding:20px;border-radius:14px;text-align:center;">
            ...
            <div style="font-size:2rem;font-weight:700;"><?php echo e($avgKwh ? number_format($avgKwh,2) : '-'); ?></div>
        </div>
        <div style="flex:1 1 220px;background:#fff0f3;padding:20px;border-radius:14px;text-align:center;">
            ...
            <div style="font-size:2rem;font-weight:700;"><?php echo e($energyProfiles->count()); ?></div>
        </div>
    </div>
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $energyProfiles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $profile): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        ...
                        <td style="padding:10px 14px;"><?php echo e($profile->baseline_kwh); ?></td>
                        ...
                        <td style="padding:10px 14px;"><?php echo e($profile->baseline_source ?? '-'); ?></td>
                        <td style="padding:10px 14px;display:flex;gap:8px;align-items:center;position:relative;">
                            <?php
                                $role = strtolower(auth()->user()->role ?? '');
                            ?>
                            <?php if($role === 'engineer' || $role === 'super admin'): ?>
                                <form method="POST" action="<?php echo e(route('energy-profile.toggle-approval', ['facility' => $facilityModel->id, 'profile' => $profile->id])); ?>" style="display:inline;position:relative;">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit" title="Toggle Engineer Approval" style="background:none;border:none;cursor:pointer;position:relative;">
                                        ...
                                        <span class="approval-tooltip" style="display:none;position:absolute;left:50%;top:-32px;transform:translateX(-50%);background:#222;color:#fff;padding:4px 12px;border-radius:6px;font-size:0.95rem;font-weight:500;white-space:nowrap;z-index:10;"><?php if($profile->engineer_approved): ?> Approved <?php else: ?> Not Approved <?php endif; ?></span>
                                    </button>
                                </form>
                            <?php else: ?>
...
function deleteEnergyProfile(btn) {
    ...
    fetch(`/modules/facilities/<?php echo e($facilityModel->id ?? 'null'); ?>/energy-profile/${profileId}`, {
        method:'DELETE',
