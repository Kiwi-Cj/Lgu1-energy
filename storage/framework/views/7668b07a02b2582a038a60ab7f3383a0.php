...
<div style="width:100%;display:flex;justify-content:flex-end;align-items:center;gap:16px;position:relative;margin-bottom:0;">
    ...
    <button id="notifBtn" style="background:#181E29;border:none;border-radius:50%;width:44px;height:44px;display:flex;align-items:center;justify-content:center;cursor:pointer;position:relative;margin-top:18px;margin-right:18px;">
        ...
        <span id="notifBadge" style="display:<?php echo e(($alerts ?? []) && count($alerts) > 0 ? 'block' : 'none'); ?>;position:absolute;top:8px;right:8px;background:#e11d48;color:#fff;font-size:0.8em;padding:2px 6px;border-radius:999px;min-width:18px;text-align:center;"><?php echo e(count($alerts ?? [])); ?></span>
    </button>
    <!-- Dropdown for alerts -->
...
<div style="max-width:1200px;margin:0 auto;">
	...
	<div style="margin-bottom:24px;">
		...
		<div style="margin-top:8px;font-size:1rem;color:#888;">
			<span id="current-date"><?php echo e(date('F j, Y')); ?></span> |
			<span id="user-role"><?php echo e(Auth::user()->role ?? 'User'); ?></span>
		</div>
	</div>
	...
	<div class="row" style="display:flex;gap:24px;flex-wrap:wrap;margin-bottom:2rem;">
		...
		<div class="card" style="flex:1 1 220px;min-width:220px;background:#f0fdf4;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(34,197,94,0.08);">
			...
			<div style="font-size:2rem;font-weight:700;margin:8px 0;"><?php echo e($totalKwh ?? '-'); ?> kWh</div>
		</div>
		<div class="card" style="flex:1 1 220px;min-width:220px;background:#fff7ed;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(234,179,8,0.08);">
			...
			<div style="font-size:2rem;font-weight:700;margin:8px 0;">₱<?php echo e($totalCost ?? '-'); ?></div>
		</div>
		<div class="card" style="flex:1 1 220px;min-width:220px;background:#fff0f3;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(225,29,72,0.08);">
			...
			<div style="font-size:2rem;font-weight:700;margin:8px 0;"><?php echo e($activeAlerts ?? '-'); ?></div>
		</div>
		<div class="card" style="flex:1 1 220px;min-width:220px;background:#e0f7fa;padding:24px 18px;border-radius:14px;box-shadow:0 2px 8px rgba(0,188,212,0.08);">
			...
			<div style="font-size:2rem;font-weight:700;margin:8px 0;"><?php echo e($ongoingMaintenance ?? '-'); ?></div>
		</div>
	</div>
	...
	<div style="margin-bottom:2rem;">
		...
		<ul style="list-style:none;padding:0;">
			<?php $__empty_1 = true; $__currentLoopData = $recentLogs ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
				<li style="margin-bottom:8px;font-size:1.05rem;color:#444;"><?php echo e($log); ?></li>
	...
	<div style="margin-bottom:2rem;display:flex;gap:18px;flex-wrap:wrap;">
		<?php if($userRole !== 'staff'): ?>
			<a href="<?php echo e(route('facilities.create')); ?>" class="btn btn-success" style="padding:10px 24px;border-radius:8px;font-weight:600;font-size:1rem;display:flex;align-items:center;gap:8px;"><span>➕</span> Add Facility</a>
		<?php endif; ?>
		<a href="<?php echo e(route('reports.monthly')); ?>" class="btn btn-primary" style="padding:10px 24px;border-radius:8px;font-weight:600;font-size:1rem;display:flex;align-items:center;gap:8px;"><span>📄</span> View Monthly Report</a>
		<?php if($userRole === 'admin'): ?>
			<a href="<?php echo e(route('settings.index')); ?>" class="btn btn-secondary" style="padding:10px 24px;border-radius:8px;font-weight:600;font-size:1rem;display:flex;align-items:center;gap:8px;"><span>⚙️</span> System Settings</a>
			<a href="<?php echo e(route('users.index')); ?>" class="btn btn-info" style="padding:10px 24px;border-radius:8px;font-weight:600;font-size:1rem;display:flex;align-items:center;gap:8px;"><span>👤</span> Manage Users</a>
		<?php endif; ?>
	</div>
	...
	<div style="margin-bottom:2rem;">
		...
		<ul style="list-style:none;padding:0;">
			<?php $__empty_1 = true; $__currentLoopData = $alerts ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alert): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
				<li style="margin-bottom:8px;font-size:1.05rem;color:#e11d48;"><?php echo e($alert); ?></li>
			<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
var energyChart = new Chart(energyCtx, {
	...
	data: {
		...
		datasets: [{
			label: 'kWh',
			data: <?php echo json_encode($energyChartData ?? [1200, 1100, 1300, 1250, 1400, 1350]); ?>,
...
var costChart = new Chart(costCtx, {
	...
	data: {
		...
		datasets: [{
			label: '₱',
			data: <?php echo json_encode($costChartData ?? [50000, 48000, 52000, 51000, 53000, 55000]); ?>,
			borderColor: '#e11d48',
...
<?php echo $__env->make('layouts.qc-admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\lgu-energy\resources\views/modules/dashboard/index.blade.php ENDPATH**/ ?>