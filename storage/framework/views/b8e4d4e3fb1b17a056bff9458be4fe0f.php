...
<div class="report card" style="max-width:900px;margin:0 auto; padding:32px 24px 32px 24px; background:#f8fafc; border-radius:18px; box-shadow:0 8px 32px rgba(37,99,235,0.09); margin-bottom:32px;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin:0 0 18px 0;">
        ...
        <a href="<?php echo e(route('energy-incidents.index')); ?>" style="background:linear-gradient(90deg,#2563eb,#6366f1);color:#fff;padding:10px 28px;font-weight:600;border:none;border-radius:10px;box-shadow:0 2px 8px rgba(31,38,135,0.1);font-size:1.05rem;transition:0.2s;text-decoration:none;">&larr; Back to Incidents</a>
    </div>
        ...
        <?php $__empty_1 = true; $__currentLoopData = $histories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $incident): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            ...
            <div class="incident-list-row" tabindex="0" onclick="showIncidentModal(<?php echo e($incident->id); ?>)">
                <div class="incident-list-main">
                    ...
                    <div class="incident-date"><?php echo e($monthLabel); ?>/<?php echo e($yearNum ?? '-'); ?></div>
                    <div class="incident-deviation"><?php echo e($dpn !== null ? number_format($dpn, 2) . '%' : '-'); ?></div>
                    <div class="incident-alert <?php echo e(strtolower($incident->alert_level ?? 'high')); ?>"><?php echo e($incident->alert_level ?? 'High'); ?></div>
                    <div class="incident-status"><?php echo e($incident->status ?? 'High Alert'); ?></div>
                    <div class="incident-detected"><?php echo e($dateDetected); ?></div>
                </div>
            </div>
            <div id="incident-modal-<?php echo e($incident->id); ?>" class="incident-modal" style="display:none;">
                <div class="incident-modal-content">
                    ...
                    <div style="margin-bottom:12px;"><b>Facility:</b> <?php echo e($incident->facility->name ?? '-'); ?></div>
                    <div style="margin-bottom:12px;"><b>Month/Year:</b> <?php echo e($monthLabel); ?>/<?php echo e($yearNum ?? '-'); ?></div>
                    <div style="margin-bottom:12px;"><b>Deviation:</b> <?php echo e($dpn !== null ? number_format($dpn, 2) . '%' : '-'); ?></div>
                    <div style="margin-bottom:12px;"><b>Alert Level:</b> <?php echo e($incident->alert_level ?? 'High'); ?></div>
                    <div style="margin-bottom:12px;"><b>Status:</b> <?php echo e($incident->status ?? 'High Alert'); ?></div>
                    <div style="margin-bottom:12px;"><b>Date Detected:</b> <?php echo e($dateDetected); ?></div>
                    <div style="margin-bottom:12px;"><b>Description:</b> <?php echo e($incident->description ?? '-'); ?></div>
                    <div style="margin-bottom:12px;"><b>Probable Cause:</b> 
                        <?php $defaultProbable = 'System-detected: Abnormal consumption pattern'; ?>
                    ...
                    <?php if(!empty($incident->attachments)): ?>
                        <div style="margin-bottom:12px;"><b>Attachments:</b> <a href="<?php echo e(asset('storage/'.$incident->attachments)); ?>" target="_blank">View</a></div>
                    <?php endif; ?>
                </div>
            </div>
...
<?php echo $__env->make('layouts.qc-admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\energy-system\resources\views/modules/energy-incident/history.blade.php ENDPATH**/ ?>